...
function block_lite_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size', 'block-lite' ),
...
add_action( 'customize_register', 'block_lite_logo_customize_register' );
...
add_filter( 'get_custom_logo', 'block_lite_logo_customize_logo_resize' );
 ...
 */
function block_lite_logo_customize_preview_js() {
	wp_enqueue_script( 'block-lite-logo-customizer', get_template_directory_uri() . '/customizer/js/logo/customize-preview.js', array( 'jquery', 'customize-preview' ), '201709081119', true );
}
add_action( 'customize_preview_init', 'block_lite_logo_customize_preview_js' );

/**
 * JS handlers for Customizer Controls
 */
function block_lite_logo_customize_controls_js() {
		wp_enqueue_script( 'block-lite-logo-customizer-controls', get_template_directory_uri() . '/customizer/js/logo/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'block_lite_logo_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function block_lite_logo_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'block_lite_logo_customize_css' );

/**
 * Testing function to remove logo_size theme mod
 */
function block_lite_logo_remove_theme_mod() {
